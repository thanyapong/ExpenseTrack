...
const FormikFileUploader = ({
    ...
    const { touched, error, value: files } = formik.getFieldMeta<FilePreview[] | undefined>(name);
                ...
                {!showAsButton ? (
                    <>
                        <DropZoneButton
                            ...
                            sx={{ width: fullWidth ? "100%" : "unset", border: { border } }}
                        >
                            <input {...getInputProps()} />
                            {icon && <Icon>{icon}</Icon>}
                            <span>
                                <Typography variant="h6">{dropzoneText}</Typography>
                            </span>
                        </DropZoneButton>
